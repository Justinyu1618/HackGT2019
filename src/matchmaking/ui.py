...
import src.landing_art.tron.tron_host as tron_host
    ...
    def on_receive_data(self, event, data):
        if event == "disconnected":
            for i in range(len(self.players)):
                if self.players[i].sid == data["sid"]:
                    del self.players[i]
                    break
        elif event == "info":
            self.sid = data["sid"]
    ...
    def run(self):
        if self.game.name == "Pong":
            ...
            while 1:
                ...
                if self.finished:
                    self.screen.erase()
                    game = self.game(self.screen, self.sio, self.host, self.match_code,
                    ...
                    self.finished = False
                    break
        elif self.game.name == "Tron":
            tron_host.main(self.screen, 1234)
        self.run()
