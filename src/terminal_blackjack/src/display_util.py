...
class DisplayTable:
	...
	def draw_card(self, card, window, loc):
		...
		if card.facedown:
			...
			window.addstr(y, x,
							card.symbol, curses.color_pair(card.color))
			window.addstr(y + h-1, x + w-1,
							card.symbol, curses.color_pair(card.color))
			window.addstr(y + int(h/2), x + int(w/2),
							card.num, curses.color_pair(card.color))
	...
	def draw_avatar(self, player, window, loc):
		...
		for i,j in avatar:
			window.addstr(y + j, x + i, player.symbol, curses.color_pair(player.color))
